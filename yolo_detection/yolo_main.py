...
import cv2
import numpy as np
from yolo_detection import print_utils
...
def create_detected_objs(video_path) :
    model = YOLO('yolo_detection/yolov8l-seg.pt')
...
def run(video_path) :
    detected_objects = create_detected_objs(video_path)
    print_utils.print_and_view_detections(detected_objects)
